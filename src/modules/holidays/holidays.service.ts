...
export class HolidaysService {
  ...
  async createHoliday(tenantId: string, dto: CreateHolidayDto, userId: string) {
  ...
  async updateHoliday(tenantId: string, holidayId: string, dto: UpdateHolidayDto, userId: string) {
  ...
  async disableHoliday(tenantId: string, holidayId: string, userId: string) {
    const holiday = await this.prisma.holiday.findFirst({
      where: { id: holidayId, tenantId },
    });
  ...
  async getHoliday(tenantId: string, holidayId: string) {
    const holiday = await this.prisma.holiday.findFirst({
      where: { id: holidayId, tenantId },
      include: {
        assignments: {
  ...
  async assignHoliday(
    ...
    if (!dto.departmentId && !dto.locationId && !dto.region) {
      throw new BadRequestException('At least one assignment criteria is required');
    }
  ...
  async removeAssignment(tenantId: string, assignmentId: string, userId: string) {
  ...
  async getHolidayAssignments(tenantId: string, holidayId: string) {
    const holiday = await this.prisma.holiday.findFirst({
      where: { id: holidayId, tenantId },
    });
  ...
  async getUpcomingHolidays(tenantId: string, departmentId?: string, locationId?: string) {
  ...
  async getAllLocations(tenantId: string) {
